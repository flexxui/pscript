...
class TestOtherBuiltins:
    ...
    def test_zip(self):
        assert (
            evalpy("for i, x in zip([1, 2, 3], [10, 20, 30]): print(i*x)")
            == "10\n40\n90"
        )
        res = "111\n222\n333"
        assert (
    ...
    def test_sorted(self):
        assert (
            evalpy("for x in sorted([1, 9, 3, 2, 7, 8, 4]): print(x)")
            == "1\n2\n3\n4\n7\n8\n9"
        )
        assert (
            evalpy("for x in reversed(sorted([1, 9, 3, 2, 7, 8, 4])): print(x)")
            == "9\n8\n7\n4\n3\n2\n1"
        )
        assert (
            evalpy("for x in sorted([1, 9, 3, 2, 7, 8, 4], key=lambda a: -a): print(x)")
            == "9\n8\n7\n4\n3\n2\n1"
        )
        assert (
            evalpy("for x in sorted([1, 9, 3, 2, 7, 8, 4], reverse=True): print(x)")
            == "9\n8\n7\n4\n3\n2\n1"
        )

        assert (
            evalpy('for x in sorted(["bb", "aa", "mm", "dd"]): print(x)')
            == "aa\nbb\ndd\nmm"
        )
        assert (
            evalpy(
                'for x in sorted(["bb", "aa", "mm", "dd"], key=lambda x: x): print(x)'
            )
            == "aa\nbb\ndd\nmm"
        )
    ...
    def test_filter(self):
        ...
        assert (
            evalpy(code + "for x in filter(f1, [-1, -2, 0, 1, 2]): print(x)") == "1\n2"
        )
        assert (
            evalpy(code + "for x in filter(None, [-1, -2, 0, 1, 2]): print(x)")
            == "-1\n-2\n1\n2"
        )
...
class TestListMethods:
    ...
    def test_sort(self):
        ...
        assert (
            evalpy("a=[3,1,4,2]; a.sort(key=lambda x: -x, reverse=True); a")
            == "[ 1, 2, 3, 4 ]"
        )

        assert (
            evalpy('a=["bb", "aa", "mm", "dd"]; a.sort(); a')
            == "[ 'aa', 'bb', 'dd', 'mm' ]"
        )
        assert (
            evalpy('a=["bb", "aa", "mm", "dd"]; a.sort(key=lambda x: x); a')
            == "[ 'aa', 'bb', 'dd', 'mm' ]"
        )
...
class TestDictMethods:
    ...
    def test_items(self):
        assert (
            nowhitespace(evalpy("d={'a':1, 'b':2, 3:3}; d.items()"))
            == "[['3',3],['a',1],['b',2]]"
        )
...
class TestStrMethods:
    ...
    def test_that_all_str_methods_are_tested(self):
        ...
        ignore = "encode decode format_map isprintable maketrans isascii removeprefix removesuffix"
        needed = needed.difference(ignore.split(" "))
