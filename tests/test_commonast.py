...
def _performance_runner():
    ...
    timeit(
        "sample with commonast.parse + comments", commonast.parse, code, comments=True
    )
    ...
    timeit(
        "minisample with commonast.parse + comments",
        commonast.parse,
        code,
        comments=True,
    )
...
def test_call_some_more():
    ...
    for arg, cls in zip(
        node.arg_nodes + node.kwarg_nodes, [Num, Name, Starred, Num, None.__class__]
    ):
        isinstance(arg, cls)
...
def test_call_even_some_more():
    ...
    assert len(node.kwarg_nodes) == 2
    for arg, cls in zip(
        node.arg_nodes + node.kwarg_nodes,
