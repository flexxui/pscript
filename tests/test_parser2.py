...
class TestConrolFlow:
    ...
    def test_for(self):
        ...
        line = nowhitespace(
            py2js("for i in range(100, 0, -1): pass", inline_stdlib=False)
        )
    ...
    def test_list_comprehensions(self):
        ...
        # Double
        code1 = (
            "[i*j for i in [-1, -2, 1, 2, 3] if i > 0 for j in [1, 10, 100] if j<100]"
        )
        assert str(eval(code1)) == normallist(evalpy(code1))
        #
        code2 = "a = " + code1 + "; a == " + code1
        ...
        assert str(eval(code1)).replace("(", "[").replace(")", "]") == normallist(
            evalpy(code1)
        )
        #
        code2 = "a = " + code1 + "; a == " + code1
...
class TestContextManagers:
    ...
    def test_with_exception(self):
        def contexttest(x):
            c = dict(
                __enter__=lambda: print("enter"), __exit__=lambda et, ev, tb: print(et)
            )
            try:
                with c:
                    print(42)
...
class TestFunctions:
    ...
    def test_function_call_keyword_only_args_and_kwargs(self):
        ...
        assert (
            evalpy(code + "foo(1, b=8)") == '[1,2,3]{"b":8}'
    ...
    def test_recursion(self=None):
        ...
        code = (
            clscode
            + "  def f(self): self.i *= 2; return self.i if self.i > 10 else self.f()\n\n"
        )
...
class TestClasses:
    ...
    def test_ensure_use_new(self):
        ...
        assert (
            evaljs(code + 'try { var m = new MyClass13(); "ok"} catch (err) { err; }')
            == "ok"
        )
        assert evaljs(code + "try { var m = MyClass13();} catch (err) { err; }") == err
        assert (
            evaljs(code + "try { MyClass13.apply(global);} catch (err) { err; }") == err
        )
        assert (
            evaljs(
                code
                + "var window = global;try { MyClass13.apply(window);} catch (err) { err; }"
            )
            == err
        )
    ...
    def test_bound_methods(self):
        ...
        assert (
            evaljs(code + "var m = new MyClass14(); var f = m.add2; f(); f(); m.a")
            == "5"
        )
        assert (
            evaljs(code + "var m = new MyClass15(); var f = m.add3; f(); f(); m.a")
            == "7"
        )
        assert (
            evaljs(
    ...
    def test_bound_funcs_in_methods(self):
        ...
        assert (
            evaljs(code + 'var m = new MyClass16(); try {m.foo3();} catch (err) {"ok"}')
            == "ok"
        )
