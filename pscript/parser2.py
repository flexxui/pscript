...
class Parser2(Parser1):
    ...
    def parse_Raise(self, node):
        ...
        # Build code to throw
        if err_cls:
            code = self.use_std_function(
                "op_error", ["'%s'" % err_cls, err_msg or '""']
            )
        else:
            code = err_msg
    ...
    def parse_If(self, node):
        if (
            True
            and isinstance(node.test_node, ast.Compare)
            and isinstance(node.test_node.left_node, ast.Name)
            and node.test_node.left_node.name == "__name__"
        ):
        ...
        if (
            True
            and isinstance(node.test_node, ast.Call)
            and isinstance(node.test_node.func_node, ast.Name)
            and node.test_node.func_node.name == "this_is_js"
        ):
        ...
        if (
            True
            and isinstance(node.test_node, ast.UnaryOp)
            and node.test_node.op == "Not"
            and isinstance(node.test_node.right_node, ast.Call)
            and isinstance(node.test_node.right_node.func_node, ast.Name)
            and node.test_node.right_node.func_node.name == "this_is_js"
        ):
            node.body_nodes = []
    ...
    def parse_For(self, node):
        ...
        if sure_is_range:  # Explicit iteration
            ...
            self._indent += 1
            code.append(
                self.lf("if (!%s.hasOwnProperty(%s)){ continue; }" % (d_seq, target[0]))
            )
    ...
    def parse_ListComp_funtionless(self, node, result_name):
        ...
        for iter, comprehension in enumerate(node.comp_nodes):
            ...
            if isinstance(comprehension.target_node, ast.Tuple):
                target = [
                    namenode.name
                    for namenode in comprehension.target_node.element_nodes
                ]
            else:
            ...
            code.append(
                "".join(cc)
                .replace("i#", prefix + "i%i" % iter)
                .replace("iter#", prefix + "iter%i" % iter)
            )
    ...
    def parse_ListComp(self, node):
        ...
        for iter, comprehension in enumerate(node.comp_nodes):
            ...
            if isinstance(comprehension.target_node, ast.Tuple):
                target = [
                    "".join(self.parse(t))
                    for t in comprehension.target_node.element_nodes
                ]
            else:
